...
async function genTokens(colorObj: Record<string, string>) {
  const tokens: string[] = [];
 ...
 * overwritten. To change theme variables, please update
 * corresponding @config files instead.
 * ========================================================
 */
`;
...
(async function generateThemes() {
  try {
    ...
    const rootTokens = await genTokens(themeConfig.colors.root);
    const darkTokens = await genTokens(themeConfig.colors.dark);
...
.dark {
  ...
  `;
    ...
    const tokensOutputPath = path.join(process.cwd(), "styles", "tokens.css");
    await fs.mkdir(path.dirname(tokensOutputPath), { recursive: true });
    await fs.writeFile(tokensOutputPath, tokens.trim() + "\n", "utf-8");
    console.log(`\n[✓] Successfully generated tokens in: ${tokensOutputPath}`);
  } catch (error) {
